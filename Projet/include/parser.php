...
$idPiece = $_GET['idPiece']; //TODO POST

// On va laisser le temps à php de turbiner 
ini_set('max_execution_time', 999);
$file = "file.txt"; // TODO POST
...
{ 
	...
	{
		echo "ERREUR: Le fichier importé contient seulement ". $nbTotalLigne . " lignes,<br> veuillez importer un fichier qui fais minimum 2 lignes (1 ligne pour les libelles de variables, une ligne pour les valeurs des variables.";
	...
	{ 
		...
		while(!feof($fp)) { // Tant qu'on n'est pas à la fin du fichier 
			...
			if($ligne == 0) // premiere ligne: on recupère les libvariables
			{
				...
				{ 
					echo 'Error : wrong date format at line ' . ($numLigneCourante + 1) . ' in your file :(...<br> Correct the line and import the file again !<br>';
					$dateErronee = true;
				} else {
				...
				{
					echo "Debug: la mesure a la date ". date_format($datetime, "Y-m-d H:i:s"). " existe deja dans la bdd ! (ligne non importee dans la bdd !)<br>";
